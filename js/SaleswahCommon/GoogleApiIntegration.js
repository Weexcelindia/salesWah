...
function callGoogle() {
    ...
    //  alert('starting');
    googleapi.authorize({
        client_id: '96075073876-m49eqog73q2bo0dkab4iomlgl5ndkkl8.apps.googleusercontent.com',
        client_secret: '7ATYMr9PWzPCpKboiloNj51e',
    ...
    $.ajax({
        ...
        success: function (data) {
         ...
         //   alert(JSON.stringify(data));
            //            localStorage.removeItem('GoogleSocialSignIn');
            //            localStorage.setItem('GoogleSocialSignIn', JSON.stringify({
            //                Email: data.email,
            //                FullName:data.name
            //            }));
            ...
            if (data.organizations != null) {
                for (var index in data.organizations) {
                    ...
                    var employerdetail = data.organizations[index];
                    var workType = data.organizations[index].type;
                    if (workType == 'work') {
                        var RegistertxtCompanyName = document.getElementById('RegistertxtCompanyName');
                        if (RegistertxtCompanyName != null) {
                            RegistertxtCompanyName.value = employerdetail.name;
                        }
                        break;
                    }
                }
                ...
                var RegistertxtCompanyName = document.getElementById('RegistertxtCompanyName');
                if (RegistertxtCompanyName != null) {
                    RegistertxtCompanyName.value = data.occupation;
                }
            }
            ...
            if (txtLogin != null) {
                ...
                //SocalSignIn('sumit@anwesha.in', 'Google account');
                SocalSignIn(data.email, 'Google account');
                //txtLogin.value = data.email;
            }
        }
    });
    disconnectUser();
